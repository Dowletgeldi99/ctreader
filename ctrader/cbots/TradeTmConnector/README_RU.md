...
## Strategy V4.3 Wave Rider на cBot Cloud

Strategy V4 работает только на DEMO-счёте. После запуска cBot передаёт backend
220 закрытых H1, 50 закрытых M15 и отдельным пакетом 300 закрытых M5 свечей
XAUUSD вместе с tick volume. Затем передаётся каждая новая закрытая свеча.
В Telegram выполните `/strategy_v4` и включите нужный cBot demo. Исторический
spread не подменяется текущим и история никогда не создаёт сделку при запуске.

H1 сохраняется для аналитики, но не блокирует BUY или SELL и не участвует во входе.
EMA 8/21 на M15 должны подтвердить TREND_UP или TREND_DOWN. Вход создаёт пробой
шести закрытых M5 при directional efficiency не ниже 0.30: тело должно занимать
не менее 55%, диапазон — 0.6–3 ATR, противоположная тень — не более 35%, а
tick volume — не ниже 0.90 от медианы. Глубина закрытия за уровнем также
обязательна. Сначала открывается Probe 0.01 lot. Main 0.01 lot добавляется только
после направленного retest/reclaim либо подтверждённого momentum. Если за шесть
M5 свечей движение не достигло +0.5R, позиция закрывается как NO_FOLLOW_THROUGH.
Структурный SL ограничен диапазоном 1.3–3 M5 ATR, TP равен 2.5R. Дневного лимита
сделок нет; одновременно допускается одна идея. LIVE-счета сервер отклоняет.
