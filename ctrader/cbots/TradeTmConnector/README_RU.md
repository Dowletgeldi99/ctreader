...
## Strategy V3 на cBot Cloud

Strategy V3 работает только на DEMO-счёте. После запуска cBot передаёт backend
...
В Telegram выполните `/strategy_v3` и включите нужный cBot demo. Исторический
spread не подменяется текущим и история никогда не создаёт сделку при запуске.

Сигнал использует EMA 20/50 на H1, структуру последних 20 M15 и breakout/retest
на M5. Проверяются тело и тени M5, сила закрытия, относительный tick volume и
окно важных USD-новостей. HIGH_VOL разрешён при качественном импульсе, а
EXTREME_VOL блокируется. Сначала открывается Probe 0.01 lot, после подтверждения
на M5 — Main 0.01 lot. Обе позиции получают расчётные SL и TP 3R. LIVE-счета
для Strategy V3 сервер отклоняет.
