...
## Strategy V2.1 на cBot Cloud

Strategy V2.1 работает только на DEMO-счёте. После запуска cBot передаёт backend
220 закрытых H1 и 50 закрытых M15 свечей XAUUSD вместе с tick volume, затем
каждую новую закрытую свечу. В Telegram выполните `/strategy_v2` и включите
нужный cBot demo. Исторический spread не подменяется текущим и история никогда
не создаёт сделку при запуске.

Сигнал использует EMA 50/200 на H1, breakout 20 на M15 и ATR 14. Дополнительно
проверяются режим рынка, тело и тени breakout-свечи, сила закрытия, относительный
tick volume и окно важных USD-новостей. Каждый breakout-кандидат сохраняется с
решением `EXECUTE` или `SKIP` для будущего ML-датасета. Сначала открывается Probe
0.01 lot, после подтверждения — Main 0.01 lot. Обе позиции получают расчётные SL
и TP 3R. LIVE-счета для Strategy V2.1 сервер отклоняет.
