  ...
  private async sendStrategyV2(ctx: Context): Promise<void> {
    ...
    const latestCandidate = configs.flatMap((config) => config.candidates)
      .filter((candidate) => typeof candidate.features === "object" && candidate.features !== null
        && !Array.isArray(candidate.features) && candidate.features.strategyVersion === "4.3")
    ...
    await ctx.reply([
      "Strategy V4.3 · XAUUSD Wave Rider · DEMO",
      "M15 EMA 8/21 direction · M5 breakout 6 · ATR 14 · H1 не участвует",
      "M15 TREND · efficiency ≥0.30 · body ≥55% · range 0.6–3 ATR · wick ≤35% · volume ≥0.90×",
      "MAIN только после сильного reclaim/momentum · no-follow-through 6 M5 · TP 2.5R · без дневного лимита",
